...
(function($){
  ...
  h1Link.css({
    "color":"#ccc", "fontWeight":"100", 'padding':'1rem',
  ...
  boxLi.css({
    'width':'100%' , 'height':'3rem' , 'margin':'auto','margin':'10px','borderBottom' : '1px solid #111',
    'backgroundColor':'#cca'
  });
  ...
  // 가급적 모든 요솓를 변수로 처리하여, 필요시 사용할 수 있도록 처리하는 것이 좋다.
  ...
  // h1 span -> $('h1 span') -> $('h1').find('span')

  //li:nth-child(2) -> $('li:nth-child(2)') -> $('li:nth(1)') -> $('li:eq(1)') -> $('li').eq(1)
  ...
  dt.css({float:'left',padding:'0.5rem', backgrouondColor:'#fdd', width:'300px'});
  dtNext.css({float:'left',fontSize:'1.2rem', color:'#f087'});
