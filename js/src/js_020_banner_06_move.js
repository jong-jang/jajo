...
// 105. 값이 - 일경우 첫요소가 나타나면 마지막요소로 다시 이동
...
// 함수
var evtPosition = function(e){
  var evt = e.originalEvent;
  var eType = e.type.slice(0,5); // touch인지 아닌지만 파악
  // 글.slice(시작위치, 갯수);
  var checkPoint;
/*
  if(eType === 'touch'){
...
*/
eType === 'touch' ? checkPoint = evt.changedTouches[0].pageX : checkPoint = evt.pageX  ;
return checkPoint;
...
viewArea.on('mousedown touchstart', function(e){
  // 이벤트 발생기준으로 위치값을 파악
  startPoint = evtPosition(e);
});
viewArea.on('mouseup touchend', function(e){
  endPoint = evtPosition(e);
